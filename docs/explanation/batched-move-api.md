...
Resetは非同期の更新操作である。完了前に次のmoveを送ると、サーバーがPlay後のstateへmoveを適用したレスポンスとResetのレスポンスが競合する。`App`はReset開始時に同期的なrefを立て、完了まで手動操作、Playback、batch準備、キーボード操作を禁止する。表示用stateも併用し、操作ボタンをdisabledにする。
